...
const api = axios.create({
    baseURL: `http://localhost:8080/api/v1/absenteeisms`
})

class Absenteeisms extends React.Component {
    state = {absenteeisms: []}
    pageNumber = 0;
    sizeNumber = 20;
    ...
    getAbsenteeisms = async () => {
        try {
            ...
            this.setState({absenteeisms: data})
        } catch (err) {
    ...
    deleteAbsenteeisms = async (id) => {
        let data = await api.delete(`/${id}`)
        this.getAbsenteeisms()
    }

    updateAbsenteeisms = async () => {
    ...
    render() {
        return (
                <div className="container-fluid m-2">
                    <h3 className="text-dark mb-4">Absenteeisms</h3>
                    <div className="card shadow">
                        <div className="card-header py-3">
                            <p className="text-primary m-0 font-weight-bold">Absenteeisms Info</p>
                        </div>
                        <div className="card-body">
                            <div className="row">
                                ...
                                <div className="col-md-6">
                                    <div className="text-md-right dataTables_filter" id="dataTable_filter"><label>
                                        <Button className={"mr-sm-2"}>Add new absenteeism</Button>
                            ...
                            <div className="table-responsive table mt-2" id="dataTable" role="grid"
                                ...
                                <Table className="table my-0" id="dataTable">
                                    ...
                                    <tbody>
                                    {this.state.absenteeisms.map(absenteeism =>
                                        <tr className={"table-responsive-sm bordered hover"}  key={absenteeism.employeeId}>
                                            <td>{absenteeism.employeeId}</td>
                                            <td>{absenteeism.firstName + " " + absenteeism.lastName}</td>
                                            <td>{absenteeism.departmentName}</td>
                                            <td>{absenteeism.absenteeismName}</td>
                                            <td>{absenteeism.dateFrom}</td>
                                            <td>{absenteeism.dateTo}</td>
                                            <td>
                                                <Button className={"mr-sm-2"} onClick={() => this.deleteAbsenteeisms(absenteeism.id)}>Delete</Button>
                                                <Button className={"mr-sm-2"} onClick={() => this.updateAbsenteeisms()}>Edit</Button>
                                            </td>
                                        </tr>)}
                                    </tbody>
                                    <tfoot>
                                    <tr>
                                        ...
                                        <td><strong>Absenteeism Name</strong></td>
                            ...
                            <div className="row">
                                <div className="col-md-6 align-self-center">
                                    <p id="dataTable_info" className="dataTables_info" role="status"
                                       aria-live="polite">Showing {this.state.absenteeisms.length} to 20 of {this.state.absenteeisms.length}</p>
                                </div>
                                <div className="col-md-6">
                                    <nav
